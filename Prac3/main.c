...
uint8_t values[] = {0b10101010,0b01010101,0b11001100,0b00110011,0b11110000,0b00001111};
uint16_t counter = 0;
...
int main(void)
{
  ...
  write_to_address(0,values[0]);
  write_to_address(1,values[1]);
  write_to_address(2,values[2]);
  write_to_address(3,values[3]);
  write_to_address(4,values[4]);
  write_to_address(5,values[5]);
...
void EXTI0_1_IRQHandler(void)
{
  ...
  if(current_time - last_time > 100){ // Debounce

	if(LED_7_speed == 2){
		htim6.Init.Period = 1000-1;
		LED_7_speed = 1;
	}
	else if(LED_7_speed == 1){
		htim6.Init.Period = 500-1;
		LED_7_speed = 2;
	}
...
void TIM16_IRQHandler(void)
{
	...
	// Get the int value to display
	uint8_t x = read_from_address(counter);
	// Check whether read is correct
	if(x >= 0){ //values[counter]){
		// Create char array
		char arr[10];
		itoa(x,arr,10);
		// Write value
...
uint32_t ADCtoCCR(uint32_t adc_val){
  // TODO: Calculate CCR value (val) using an appropriate equation
	uint32_t val = adc_val*(47999/4096);//Scale by ARR divide by max 12 bit value
	return val;
}
